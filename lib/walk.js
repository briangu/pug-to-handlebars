...
function walk (opts, obj) {
  ...
  if (obj.type === 'Include') {
    return '<!-- TODO: Fix unsupported pug include -->\n' +
      'include ' + obj.file.path + '\n';
  }

  if (obj.type === 'Extends') {
    return '<!-- TODO: Fix unsupported pug extends -->\n' +
...
function mixin (opts, obj) {
  return '<!-- TODO: Fix unsupported pug mixin -->\n' +
...
function text (obj) {
  ...
  if (/\!\{.+\}/.test(obj.val)) {
    result = '<!-- TODO: Fix unsupported pug inline javascript -->\n' + result;
  }
